...
final class MeetingDetector {
    ...
    static func bestOutputAudioProcess(for app: DetectedApp,
        ...
        // models that `<host>.` relationship for every host, so honour it here
        // as a fallback — without it `hasOutputAudio` stays false, the detector
        // never surfaces the app, and the process tap is skipped entirely,
        // leaving the meeting recorded mic-only. The host keeps precedence so
        // apps whose main process emits audio are unaffected.
        //
        // This is the point the two changes meet: matching the namespace is
        // what puts `modulehost` in front of the detector, and `modulehost`
        // holds its streams open for the whole life of the app. Every clause
        // that can reach a sibling therefore has to skip the always-open ones,
        // or Teams reads as permanently in a meeting.
